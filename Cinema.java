...
public class Cinema {
    ...
    public Cinema(int rows, int seats) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < seats; j++) {
                this.seats[i][j] = "O"; // Alle sæder er "O" som standard
            }
        }
        this.seats[0][0] = "X"; // Direktørens sæde
    }
    ...
    public boolean cancelReservation(int row, int seat) {
        if (seats[row][seat].equals("X")) { // Skift fra "x" til "X"
            seats[row][seat] = "O";
            return true;
        }
        return false;
    ...
    public String toString() {
        String result = ""; // Initialiser som tom streng
